...
class ControlledSystemDoublePendulum(nn.Module):
    ...
    def _dynamics(self, q, p, u):
        ...
        DET = (l1 ** 2 * l2 ** 2 * m2 ** 2 * (
            -torch.cos(q2) ** 2) + l1 ** 2 * l2 ** 2 * m2 ** 2 + l1 ** 2 * l2 ** 2 * m1 * m2)

        dq1dt = (l2 ** 2 * m2 * p1) / DET - (l2 * m2 * p2 * (l1 * torch.cos(q2) + l2)) / DET
        dq2dt = (p2 * (2 * l2 * l1 * m2 * torch.cos(q2) + l1 ** 2 * (m1 + m2) + l2 ** 2 * m2)) / DET - (
                    l2 * m2 * p1 * (l1 * torch.cos(q2) + l2)) / DET

        dp1dt = (-g) * (torch.sin(q1 + q2) * l2 * m2 + torch.sin(q1) * l1 * (m1 + m2)) + u1
        dp2dt = k1 * (torch.pi - 2 * q2) - l2 * m2 * (
                    g * torch.sin(q1 + q2) - torch.sin(q2) * l1 * dq1dt * (dq1dt + dq2dt)) + u2
...
class StabilizedSystemDoublePendulum(nn.Module):
    # Elastic Double Pendulum Model
    def __init__(self, V, a_M=torch.tensor(0.0), a_E=torch.tensor(0.0),
                 x_fun=lambda t: torch.tensor((0, 0, 0, 0)).float(),
                 x_num=torch.tensor(((0, 0, 0, 0), (0, 0, 0, 0))).float()):
        super().__init__()
        self.V, self.T, self.n = V, torch.nn.Parameter(torch.tensor([1.0])), 2
