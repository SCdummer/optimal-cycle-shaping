...
class ControlledSystemDoublePendulum(nn.Module):
    ...
    def _dynamics(self, q, p, u):
        ...
        dp2dt = -((1/(l1**2 * l2**2 * (m1+torch.sin(q2)**2 * m2)**2)) * ((-p2**2) * torch.sin(q2) * (torch.cos(q2)*l1+l2) * (cos(q2)*l2*m2+l1*(m1+m2))\ 
        + (1/2)* p1 * p2 * torch.sin(q2) * l2 * (4*cos(q2)*l2*m2+l1*(2*m1+(3+cos(2*q2))*m2))\ 
        + l2**2 * ((-cos(q2)) * p1**2 * torch.sin(q2) * m2 + l1**2 * (m1+torch.sin(q2)**2 * m2)**2 *(g*torch.sin(q1+q2)*l2*m2-k1/2*(pi-2*q2)))))\ 
        + u2
